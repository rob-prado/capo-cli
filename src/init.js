...
export function isValidProjectName(name) {
    if (typeof name !== 'string' || name.length === 0) return 'Project name cannot be empty.';
...
export function isValidBrand(brand) {
    if (typeof brand !== 'string' || brand.length === 0) return 'Brand name cannot be empty.';
...
export function initProject(projectName, initialBrand) {
    ...
    if (isValidProjectName(projectName) !== true) {
        console.error(chalk.red('Fatal: Invalid projectName. Must be alphanumeric and underscores only.'));
        process.exit(1);
    }

    if (isValidBrand(initialBrand) !== true) {
        console.error(chalk.red('Fatal: Invalid initialBrand. Must be alphanumeric only.'));
        process.exit(1);
    }
