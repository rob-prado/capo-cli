...
import { isValidBrand } from './init.js'
...
/**
 * Main workflow entry function for "Create Brand".
 *
 * @param {string} brandName - Optional brand name provided via CLI flags.
 */
export async function createBrand(brandName) {
  ...
  if (brandsData.brands.includes(finalBrand)) {
    console.error(
      chalk.red(`Fatal: Brand '${finalBrand}' already exists in brands.json.`),
    )
    process.exit(1)
  }
