...
if [ -d "${IOS_PROJ_DIR}" ]; then
	...
	# 2. Rename Colorset
	COLORSET=$(ls -d "${IOS_PROJ_DIR}/Colors.xcassets"/BootSplashBackground-*.colorset 2>/dev/null | head -n 1)
	if [ -n "${COLORSET}" ]; then
		mv "${COLORSET}" "${IOS_PROJ_DIR}/Colors.xcassets/LaunchScreenBackground.colorset"
	fi

	# 3. Rename Imageset & Files
	IMAGESET=$(ls -d "${IOS_PROJ_DIR}/Images.xcassets"/BootSplashLogo-*.imageset 2>/dev/null | head -n 1)
	if [ -n "${IMAGESET}" ]; then
