...
4. **Environment File Generation**: Scaffolds empty `.env.$BRAND.{dev,staging,prd}` files if they do not exist to prevent Xcode build failures during pre-actions.
5. **Xcode Scheme Cloning**: Duplicates `defaultDev.xcscheme` and `defaultProd.xcscheme`, injects the brand identifiers inside the `.env` references, and permanently maps them into the active iOS `xcshareddata/xcschemes` folder so Xcode recognizes the new flavor targets.
